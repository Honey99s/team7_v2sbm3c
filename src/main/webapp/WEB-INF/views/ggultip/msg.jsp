...
<c:set var="ggultipno" value="${param.ggultipno }" /> <%-- mav.addObject("modelno", carVO.getmodelno()); // redirect parameter 적용 --%>
    ...
    <UL>
      <c:choose>
        ...
        <c:when test="${code == 'create_success'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_success">새로운 꿀팁을 등록했습니다.</span>
          </LI> 
        </c:when>
        
        <c:when test="${code == 'create_fail'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_fail">새로운 꿀팁 등록에 실패했습니다.</span>
          </LI>                                                                      
        </c:when>
        
        <c:when test="${code == 'update_fail'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_fail">꿀팁 수정에 실패했습니다.</span>
          </LI>                                                                      
        </c:when>
        
        <c:when test="${code == 'delete_success'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_success">꿀팁 삭제에 성공했습니다.</span>
          </LI>                                                                      
        </c:when>        
        
        <c:when test="${code == 'delete_fail'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_fail">꿀팁 삭제에 실패했습니다.</span>
          </LI>                                                                      
        </c:when> 
      ...
      <li class='li_none'>
        ...
        <button type='button' onclick="location.href='./create.do?ggultipno=${ggultipno}'" class="btn btn-secondary btn-sm">새로운 꿀팁 등록</button>
