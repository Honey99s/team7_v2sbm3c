    ...
    <ul>
      <c:choose>
        <c:when test="${param.code == 'create_success'}"> <%-- Java if --%>
          <li class='li_none'>
            <span class="span_success">${param.cname }님(${param.id }) 고객 가입을 축하합니다.</span>
          </li>  
          <li class='li_none'>
            <button type='button' 
        ...
        <c:when test="${param.code == 'create_fail'}"> <%-- Java if --%>
          <li class='li_none'>
            <span class="span_fail">고객 가입에 실패했습니다. 다시 시도해주세요.</span>
          </li>                                                                      
        </c:when>

        <c:when test="${param.code == 'duplimodel_fail'}"> <%-- Java if --%>
          <li class='li_none'>
            <span class="span_fail">ID가 이미 사용중입니다. 고객 가입에 실패했습니다. <br>다시 시도해주세요.</span>
          </li>                                                                      
        </c:when>
        
        <c:when test="${param.code == 'update_success'}"> <%-- Java if --%>
          <li class='li_none'>
            <span class="span_success">${param.cname }님(아이디: ${param.id }) 고객 정보를 변경했습니다.</span>
          </li>
          <li class='li_none'>
            <button type='button' 
        ...
        <c:when test="${param.code == 'update_fail'}"> <%-- Java if --%>
          <li class='li_none'>
            <span class="span_fail">${param.cname }님(${param.id }) 고객 정보 수정에 실패했습니다.</span>
          </li>                                                                      
        </c:when>
        ...
        <c:when test="${param.code == 'passwd_update_success'}"> <%-- Java if --%>
          <li class='li_none'>
            <span class="span_success">${param.cname }님(${param.id }) 패스워드를 변경했습니다.</span>
          </li>   
          <li class='li_none'>
            <button type='button' 
                         onclick="location.href='/'"
      ...
      <li class='li_none'>
        ...
        <%-- <a href="./list_by_modelno.do?modelno=${param.modelno}" class="btn btn-primary">목록</a> --%>
        <%-- <button type='button' onclick="location.href='./list_by_modelno_search.do?modelno=${param.modelno}'" class="btn btn-primary">목록</button> --%>
        <%-- <button type='button' onclick="location.href='./list_by_modelno_search_paging.do?modelno=${param.modelno}'" class="btn btn-primary">목록</button> --%>
