...
window.onload = function() {
  ...
  function checkID() {
    ...
    if (id.value.trim().length == 0) {
      ...
      $.ajax({
        url: './checkID.do', // spring execute, http://localhost:9091/customer/checkID.do?id=user1@gmail.com
        type: 'get',  // post
      ...
      // 처리중 출력
      id_msg.innerHTML="<img src='/customer/images/ani04.gif' style='width: 6%;'>"; // static 기준
      
    }
  }

  function send() { // 고객 수정 처리
	    let id = document.getElementById('id');
	    let id_msg = document.getElementById('id_msg');
...
<c:import url="/menu/top.do" />
  <DIV class='title_line'>고객 정보 조회 및 수정(*: 필수)</DIV>
  ...
  <FORM name='frm' id='frm' method='POST' action='./update.do' class="">
    ...
    <div class="form-group"> <%-- 줄이 변경되지 않는 패턴 --%>
      <label>아이디*:
        <input type='text' class="form-control form-control-sm" name='id' id='id' value='${customerVO.id }' required="required" placeholder="아이디*" autofocus="autofocus">
      </label>
    ...
    <div class="form-group"> <%-- label의 크기에따라 input 태그의 크기가 지정되는 형태 --%>
      <label>성명*:
        <input type='text' class="form-control form-control-sm" name='cname' id='cname' value='${customerVO.cname }' required="required" placeholder="성명">
      </label>
    ...
    <div class="form-group"> <%-- label의 크기에따라 input 태그의 크기가 지정되는 형태, 줄이 변경되지 않는 패턴 --%>
      <label>전화 번호:
        <input type='text' class="form-control form-control-sm" name='tel' id='tel' value='${customerVO.tel }' required="required" placeholder="전화번호">
      </label>
      예) 010-0000-0000
    </div>   
    ...
    <div class="form-group">
      <label>우편 번호:
        <input type='text' class="form-control form-control-sm" name='zipcode' id='zipcode' value='${customerVO.zipcode }' placeholder="우편번호">
      </label>
    ...
    <div class="form-group">
      ...
      <input type='text' class="form-control form-control-sm" name='address1' id='address1' value='${customerVO.address1 }' placeholder="주소">
    </div>   

    <div class="form-group">
      <label style="width: 100%;">상세 주소:</label>
      <input type='text' class="form-control form-control-sm" name='address2' id='address2' value='${customerVO.address2 }' placeholder="상세 주소">
    </div>   
