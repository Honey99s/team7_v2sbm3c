...
public class CustomerCont {
  ...
  */
  @ResponseBody
  @RequestMapping(value="/customer/checkID.do", 
  ...
  */
  ...
  /**
   * 회원 조회
   * http://localhost:9092/customer/read.do?customerno=1
   * 관리자, 회원 본인만 가능
   * @param customerno
   * @return
   */
  ...
  public ModelAndView read(HttpSession session, HttpServletRequest request){
    ...
    int customerno = 0;
    if (this.customerProc.isCustomer(session) || this.masterProc.isMaster(session)) { 
      // 로그인한 경우
